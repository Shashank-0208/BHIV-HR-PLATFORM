...
async def test_communication_system(
    ...
    try:
        ...
        if channel == "email":
            response = await httpx.post(f"{langgraph_service_url}/test/send-email",
                                      params=test_data,
                                      headers={"Authorization": f"Bearer {api_key_secret}"},
                                      timeout=10.0)
        elif channel == "whatsapp":
            response = await httpx.post(f"{langgraph_service_url}/test/send-whatsapp", 
                                      params=test_data,
                                      headers={"Authorization": f"Bearer {api_key_secret}"},
                                      timeout=10.0)
        elif channel == "telegram":
            response = await httpx.post(f"{langgraph_service_url}/test/send-telegram",
                                      params=test_data, 
                                      headers={"Authorization": f"Bearer {api_key_secret}"},
                                      timeout=10.0)
        else:
